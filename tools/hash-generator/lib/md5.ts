...
export function md5(data: Uint8Array): string {
  ...
  for (i = 64; i <= n; i += 64) {
    md5cycle(state, md5blk(data.subarray(i - 64, i)));
  }

  const tail = new Uint8Array(64);
  const remaining = n - (i - 64);
  tail.set(data.subarray(i - 64, n));
  tail[remaining] = 0x80;
  ...
  // For files > 512MB we'd need the high 32 bits, but this covers practical use
  tail[60] = 0; tail[61] = 0; tail[62] = 0; tail[63] = 0;
