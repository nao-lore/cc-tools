...
export function getHueCategory(hex: string): HueCategory {
  const { r, g, b } = hexToRgb(hex);
  ...
  // Dark brownish: low saturation reds/oranges with low luminance
  const saturation = max === 0 ? 0 : delta / max;
  if (saturation < 0.5 && luminance < 100 && hue < 40) return "brown";
